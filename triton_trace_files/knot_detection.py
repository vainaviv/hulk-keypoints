...
class KnotDetector:
    ...
    #uses self.crossings.
    #TODO: add in intermediate crossings rules?

    def find_knot(self):
        self.crossings_in_consideration = self.crossings
        self.knot = self.find_knot_helper()
        return self.knot
    
    def find_knot_helper(self):
        
        if len(self.crossings_in_consideration) < 3:
            return None
        
        for cross_idx in range(len(self.crossings_in_consideration)):
            crossing = self.crossings_in_consideration[cross_idx]
            next_cross_idx = self.find_later_crossing_idx(crossing, self.crossings_in_consideration, cross_idx + 1)
    
            # no previous occurrence of this crossing found later
            if next_cross_idx == -1:
                continue

            # previous occurrence is right before this (trivial loop!)
            elif next_cross_idx - cross_idx == 1:
                self.crossings_in_consideration = self.crossings_in_consideration[next_cross_idx + 1: ]
                return self.find_knot_helper()

            else:
                # checks if each crossing in between is paired off
                if self.check_if_all_pairs(self.crossings_in_consideration[cross_idx + 1: next_cross_idx]):
                    self.crossings_in_consideration = self.crossings_in_consideration[cross_idx + 1: next_cross_idx]
                    return self.find_knot_helper()

                else:
                    #checks if first crossing in knot is under 
                    if self.crossings_in_consideration[cross_idx]['ID'] == 0: 
                        return self.crossings_in_consideration[cross_idx:next_cross_idx + 1]
                    else:
                        #TODO check this logic - for if first is over 
                        self.crossings_in_consideration = self.crossings_in_consideration[cross_idx + 1:]
                        return self.find_knot_helper()
    ...
    #finds if there is a later occurrence of a crossing in crossings_in crossing_index (from pos start onwards)
    def find_later_crossing_idx(self, crossing, crossings_in_consideration, start_idx):
        '''
        Returns the index of the next sighting of crossing on the stack (from start onwards)
...
def test_knot_detector_overhand_loops_o_first(detector):
    ...
    assert detector.find_knot() == detector.crossings[2:6]
...
if __name__ == '__main__':
    ...
    #TODO add reidemeister move 2 - currently failing these tests bc that
